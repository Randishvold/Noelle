...
class MessageHandlerCog(commands.Cog, name="AI Message Handler"):
    ...
    async def _handle_gemini_response_processing(self, target, response_obj: genai_types.GenerateContentResponse, context_prefix: str, is_interaction: bool):
        """Helper untuk memproses dan mengirim respons, dipanggil oleh on_message dan mention."""
        response_text = ""
        ...
        interaction_fup: discord.Interaction | None = None
        initial_sender_for_error = None # Untuk pesan error/kosong

        if is_interaction:
            interaction_fup = target
        ...
        if not response_text.strip():
            ...
            return
        try:
            is_companion = (context_prefix == "Info Tambahan Gambar")
            await ai_utils.send_text_in_embeds(
                target_channel=target.channel, # Selalu kirim ke channel
                response_text=response_text,
                footer_text=footer_txt,
                reply_to_message=reply_msg if not is_companion else None, 
                interaction_to_followup=interaction_fup if not is_companion else None, 
                is_direct_ai_response=not is_companion,
                custom_title_prefix=None if is_companion else "Respons Noelle" # Judul kustom jika bukan direct AI response
            )
            _logger.info(f"({context_prefix}) Respons teks selesai diproses via ai_utils.")
        except Exception as e:
            _logger.error(f"({context_prefix}) Error besar saat _handle_gemini_response_processing: {e}", exc_info=True)
    ...
    async def ai_channel_message_listener(self, message: discord.Message):
        ...
        async with message.channel.typing():
            try:
                ...
                # --- SYSTEM INSTRUCTION UNTUK SESI CHAT ---
                chat_session_config = genai_types.GenerateContentConfig(
                    system_instruction=DEFAULT_SYSTEM_INSTRUCTION
                )
                # -------------------------------------------
                api_response = await asyncio.to_thread(chat_session.send_message, 
                                                       message=user_input_parts_for_api,
                                                       config=chat_session_config) # Terapkan config
                ...
                await self._handle_gemini_response_processing(message, api_response, context_log_prefix, is_interaction=False)
...
async def setup(bot: commands.Bot):
    ...
    if client is None or not gemini_services.is_ai_service_enabled():
        _logger.error("MessageHandlerCog: Klien Gemini tidak siap. Cog tidak dimuat.")
        return
